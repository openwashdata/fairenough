...
process_data <- function(
  ...
  if (verbose) {
    cli::cli_alert_info("Found {length(data_files)} file{?s} to process")
  }
  ...
  for (file_path in data_files) {
    ...
    if (verbose) {
      cli::cli_alert_info("Processing {.file {basename(file_path)}}")
    }
    ...
    tryCatch(
      {
        ...
        if (verbose) {
          cli::cli_alert_success(
            "Processed {.file {file_name}}: {nrow(data)} rows, {ncol(data)} columns"
          )
        }
      },
      error = function(e) {
        cli::cli_alert_danger(
          "Failed to process {.file {basename(file_path)}}: {e$message}"
        )
      }
    )
  }
  ...
  if (verbose) {
    cli::cli_alert_success(
      "Processing complete: {length(processed_files)}/{length(data_files)} files"
    )
    cli::cli_alert_info(
      "Next step: Run {.fn document} to generate documentation"
    )
  }
...
process_clean <- function(
  ...
  if (verbose) {
    cli::cli_alert_success(
      "Data cleaned: {original_rows} rows, {original_cols} columns"
    )
  }
...
.export_rda <- function(
  ...
  if (file.exists(rda_path) && !overwrite) {
    if (verbose) {
      cli::cli_alert_info("Skipped {.file {name}.rda} (already exists)")
    }
  ...
  if (verbose) {
    cli::cli_alert_success("Exported {.file data/{name}.rda}")
  }
...
.export_csv <- function(
  ...
  if (file.exists(csv_path) && !overwrite) {
    if (verbose) {
      cli::cli_alert_info("Skipped {.file {name}.csv} (already exists)")
    }
  ...
  if (verbose) {
    cli::cli_alert_success("Exported {.file inst/extdata/{name}.csv}")
  }
