...
.gendict_display_progress <- function(
  ...
  if (!is.null(current_index)) {
    ...
    cli::cli_alert_success(
      "{col_names[current_index]}: {responses[current_index]}"
    )
    ...
    if (current_index %% 5 == 0 || current_index == length(col_names)) {
      ...
      cli::cli_alert_info(
        "Progress: {current_index}/{length(col_names)} ({pct}%)"
      )
    }
  } else {
    # Display all results
    ...
    cli::cli_alert_info(
      "Completed: {length(col_names)}/{length(col_names)} (100%)"
    )
  }
}
    ...
    response <- tryCatch(
      ...
      error = function(e) {
        cli::cli_alert_warning(
          "Failed to get response for {col_names[i]}: {e$message}"
        )
...
.gendict_parallel_chat <- function(chat, prompts, col_names) {
  ...
  llm_responses <- tryCatch(
    ...
    error = function(e) {
      cli::cli_alert_warning(
        "parallel_chat failed: {e$message}. Falling back to sequential processing."
      )
...
.gendict_sequential_fresh <- function(chat_config, prompts, col_names) {
  ...
  cli::cli_alert_info(
    "Processing {length(prompts)} columns (fresh connection each time)..."
  )
...
.add_examples_to_descriptions <- function(
  descriptions,
  sample_data,
  col_names
) {
...
gendict <- function(
  ...
  if (test_llm_connection) {
    ...
    if (!is.null(test_response)) {
      cli::cli_alert_success(
        "LLM connection test: {substr(test_response, 1, 50)}..."
      )
    } else {
  ...
  for (i in seq_along(col_names)) {
    ...
    if (!is.null(context)) {
      ...
      cli::cli_alert_warning(
        "Use {.code context = 'Detailed description of the data.'} to give more context to the LLM"
      )
    }

    # Adjust instructions based on whether variable is continuous
    variable_type <- if (is_cont) {
      "continuous numeric"
    } else {
