...
prompt4metadata <- function(
  ...
  if (is.null(communities) && interactive) {
    use_communities <- prompt_yes_no(
      "Add to Zenodo communities?",
      default = TRUE
    )
    if (use_communities) {
      communities_input <- prompt_field(
  ...
  if (interactive && (is.null(temporal_start) || is.null(temporal_end))) {
    has_temporal <- prompt_yes_no(
      "Does the data have temporal coverage?",
      default = TRUE
    )
    if (has_temporal) {
      temporal_start <- prompt_field(
        "Start date (YYYY or YYYY-MM-DD)",
        temporal_start
      )
      temporal_end <- prompt_field(
  ...
  if (interactive && is.null(spatial_description)) {
    has_spatial <- prompt_yes_no(
      "Does the data have geographic coverage?",
      default = FALSE
    )
    if (has_spatial) {
      spatial_description <- prompt_field(
